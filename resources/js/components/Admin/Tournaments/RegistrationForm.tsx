// components/Admin/Tournaments/RegistrationForm.tsx
...
export function RegistrationForm({ values, errors, onChange, users, isEditing }: RegistrationFormProps) {
    if (isEditing) {
        return (
    ...
    return (
        <div className="space-y-4">
            {/* Tipo de usuario */}
            <div>
                <label className="text-t-primary mb-2 block text-sm font-medium">Tipo de usuario</label>
                <div className="flex gap-4">
                    <label className="flex items-center gap-2">
                        <input
                            ...
                            className="border-border-primary text-accent focus:ring-accent h-4 w-4 focus:ring-2"
                        />
                        <span className="text-t-primary text-sm">Usuario existente</span>
                    </label>
                    <label className="flex items-center gap-2">
                        <input
                            type="radio"
                            ...
                            className="border-border-primary text-accent focus:ring-accent h-4 w-4 focus:ring-2"
                        />
                        <span className="text-t-primary text-sm">Nuevo usuario</span>
                    </label>
                </div>
            </div>
            ...
            {values.user_selection_type === 'existing' ? (
                <FormField label="Usuario" required error={errors.user_id} fullWidth>
                    {users && users.length > 0 ? (
                        ...
                        <p className="text-t-muted text-sm italic">No hay usuarios disponibles para seleccionar.</p>
                    )}
                </FormField>
            ) : (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
